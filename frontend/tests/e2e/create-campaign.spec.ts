...
test.describe('CreateCampaignPage - Unauthenticated Redirect', () => {
  test.skip('redirects unauthenticated user to login when accessing /c/:sharedCampaignCode', async ({ page }) => {
...
test.describe('CreateCampaignPage - Prefill Mode', () => {
  ...
  test.skip('shows warning about sharing access', async ({ page }) => {
    ...
    await expect(page.getByText(/read access to ALL current and future campaigns/i)).toBeVisible();
  });
});
